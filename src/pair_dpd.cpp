...
void PairDPD::compute(int eflag, int vflag)
{
  ...
  double rsq,r,rinv,dot,wc,wr,randnum,factor_dpd;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
       ...
       if (rsq < cutsq[itype][jtype]) {
        ...
        double frac_gamma=1.0;
        double disth;
        if(itype==wflag && jtype != wflag){
         disth = abs(x[j][2] - w_surface);
        	if(disth>0.01){
         	
         	frac_gamma=1+0.187*(cut[itype][jtype]/disth-1.0)-0.093*pow((1-disth/cut[itype][jtype]),3);
        }
        else {
        	frac_gamma=19.423;
        	}
      }
        else if(itype!=wflag && jtype == wflag){
        	disth = abs(x[i][2] - w_surface);
        	if(disth>0.01){
        ...
        fpair -= frac_gamma*gamma[itype][jtype]*wr*wr*dot*rinv;
        fpair += sqrt(frac_gamma)*sigma[itype][jtype]*wr*randnum*dtinvsqrt;
...
void PairDPD::settings(int narg, char **arg)
{
  ...
  wflag = force->inumeric(FLERR,arg[3]);
  w_surface = force->numeric(FLERR,arg[4]);
...
void PairDPD::coeff(int narg, char **arg)
{
  ...
  if (narg > 4) cut_one = force->numeric(FLERR,arg[4]);
