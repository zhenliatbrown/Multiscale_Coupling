...
AtomStyle(dpd/angle/meso,AtomVecDPDAngle)

#else

#ifndef LMP_MESO_ATOM_VEC_DPD_ANGLE
#define LMP_MESO_ATOM_VEC_DPD_ANGLE
...
class AtomVecDPDAngle: public AtomVecAngle, public AtomVecDPDMolecular
{
public:
    AtomVecDPDAngle(LAMMPS *);
    ~AtomVecDPDAngle() {}
...
protected:
    ...
    void pack_angle(CUDAStream stream);

    std::vector<int> nangle_prefix;
    HostScalar<int2> hst_angle_id;
    HostScalar<int4> hst_angle_packed;
    DeviceScalar<int2> dev_angle_id;
    DeviceScalar<int4> dev_angle_packed;
};
