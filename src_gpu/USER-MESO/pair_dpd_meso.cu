...
MesoPairDPD::MesoPairDPD( LAMMPS *lmp ) : Pair( lmp ), MesoPointers( lmp ),
    dev_coefficients( lmp, "MesoPairDPD::dev_coefficients" )
{
    split_flag  = 1;
    coeff_ready = false;
    random = NULL;
...
void MesoPairDPD::prepare_coeff()
{
        ...
        if( i >= p_beg ) {
            ...
            for( int p = part_id; p < n_pair; p += n_part ) {
                ...
                if( rsq < coeff_ij[p_cutsq] && rsq >= EPSILON_SQ ) {
                    ...
                    r64 wr       = __powd( wc, coeff_ij[p_expw] );
...
void MesoPairDPD::compute_kernel( int eflag, int vflag, int p_beg, int p_end )
{
    ...
    if( eflag || vflag ) {
        ...
        static GridConfig grid_cfg = meso_device->configure_kernel( gpu_dpd<1>, shared_mem_size );
        gpu_dpd<1> <<< grid_cfg.x, grid_cfg.y, shared_mem_size, meso_device->stream() >>> (
        ...
        // evaluate force only
        static GridConfig grid_cfg = meso_device->configure_kernel( gpu_dpd<0>, shared_mem_size );
        gpu_dpd<0> <<< grid_cfg.x, grid_cfg.y, shared_mem_size, meso_device->stream() >>> (
            meso_atom->tex_coord_merged, meso_atom->tex_veloc_merged,
            meso_atom->dev_force(0),   meso_atom->dev_force(1),   meso_atom->dev_force(2),
            meso_atom->dev_virial(0), meso_atom->dev_virial(1), meso_atom->dev_virial(2),
            meso_atom->dev_virial(3), meso_atom->dev_virial(4), meso_atom->dev_virial(5),
...
void MesoPairDPD::compute_bulk( int eflag, int vflag )
{
...
void MesoPairDPD::compute_border( int eflag, int vflag )
{
    int p_beg, p_end, c_beg, c_end;
...
void MesoPairDPD::compute( int eflag, int vflag )
{
    int p_beg, p_end, c_beg, c_end;
...
uint MesoPairDPD::seed_now() {
    return premix_TEA<64>( seed, update->ntimestep );
}

void MesoPairDPD::settings( int narg, char **arg )
{
...
void MesoPairDPD::coeff( int narg, char **arg )
{
    if( narg < 6 || narg > 7 )
...
double MesoPairDPD::init_one( int i, int j )
{
    if( setflag[i][j] == 0 )
...
void MesoPairDPD::write_restart( FILE *fp )
{
...
void MesoPairDPD::read_restart( FILE *fp )
{
...
void MesoPairDPD::write_restart_settings( FILE *fp )
{
...
void MesoPairDPD::read_restart_settings( FILE *fp )
{
    if( comm->me == 0 ) {
...
double MesoPairDPD::single( int i, int j, int itype, int jtype, double rsq,
