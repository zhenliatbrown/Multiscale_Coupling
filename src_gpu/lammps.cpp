...
LAMMPS::LAMMPS(int narg, char **arg, MPI_Comm communicator)
{
  ...
  int meso_gpu = 0;
  int meso_gpu_skip = 0;
  bool meso_gpu_skipmode = false;
  bool mesp_gpu_selected = false;
  std::string meso_profile;
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"-partition") == 0 ||
      ...
      iarg += 3;
    } else if (strcmp(arg[iarg],"-meso") == 0 ) {
      if (iarg+2 > narg)
        error->universe_all(FLERR,"Invalid command-line argument");
      if (strcmp(arg[iarg+1],"on") == 0) mesoflag = 1;
      else if (strcmp(arg[iarg+1],"off") == 0) mesoflag = 0;
      else error->universe_all(FLERR,"Invalid command-line argument");
      iarg += 2;
    } else if (strcmp(arg[iarg],"-device") == 0) {
      if (iarg+2 > narg) error->universe_all(FLERR,"Invalid command-line argument");
      if ( arg[iarg+1][0] == '-' ) {
		meso_gpu_skipmode = true;
		meso_gpu_skip = fabs( atoi( arg[iarg+1] ) );
	  } else {
		meso_gpu_skipmode = false;
		meso_gpu = atoi( arg[iarg+1] );
	  }
	  mesp_gpu_selected = true;
	  iarg += 2;
    } else if (strcmp(arg[iarg],"-profile") == 0) {
      if (iarg+2 > narg) error->universe_all(FLERR,"Invalid command-line argument");
      meso_profile = arg[iarg+1];
      iarg += 2;
  ...
  if (mesoflag == 0) {
    ...
    if ( !mesp_gpu_selected || meso_gpu_skipmode )
	{
	  char* var;
	  int dev_count = 0, local_rank = 0;
      ...
      #endif
	  if ( meso_gpu_skipmode )
	  {
		meso_gpu = 0;
		if ( meso_gpu == meso_gpu_skip ) local_rank++;
		while( local_rank-- > 0 ) {
			meso_gpu = (++meso_gpu) % dev_count;
			if ( meso_gpu == meso_gpu_skip ) local_rank++;
		}
	  }
	  else meso_gpu = local_rank % dev_count;
	}

    if (mesoflag == 1) {
    meso_device = new MesoDevice( this, meso_gpu, meso_profile );
    if (meso_device->dummy)
      error->all(FLERR,"Cannot use -meso on without USER-MESO installed");
    } else {
	  meso_device = new MesoDevice( this, meso_gpu, meso_profile );
      if (meso_device->dummy || mesoflag == 0) {
        delete meso_device;
        meso_device = NULL;
      }
    }
...
LAMMPS::~LAMMPS()
{
  ...
  if (meso_device) delete meso_device;
  delete cuda;
  delete [] suffix;
...
void LAMMPS::create()
{
  ...
  else if (meso_device) comm = new MesoComm(this);
  else comm = new Comm(this);

  if (cuda) 
  {
    neighbor = new NeighborCuda(this);
  }
  else if (meso_device)
  {
    neighbor = new MesoNeighbor(this);
  }
  else 
  {
  ...
  else if (meso_device) domain = new MesoDomain(this);
#ifdef LMP_USER_OMP
  ...
  if (meso_device) atom = new MesoAtom(this);
  ...
  if (meso_device) timer = new MesoTimer(this);
  else timer = new Timer(this);
}
